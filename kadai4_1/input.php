  ...
  <body>
    ...
    <form action="input.php" method="POST">
        ...
        <?php echo $comment_table_fields[9] ?><select name="town_double_zip_code" id="town_double_zip_code" size="1">
            <option value="1" <?php echo ($_SESSION["town_double_zip_code"]) == 1 ? 'selected' : ''; ?>> 該当</option>
            <option value="0" <?php echo ($_SESSION["town_double_zip_code"]) == 0 ? 'selected' : '' ?>> 該当せず</option>
        </select><br />
        <?php echo $comment_table_fields[10] ?><select name="town_multi_address" id="town_multi_address" size="1">
            <option value="1" <?php echo ($_SESSION["town_multi_address"])  == 1 ? 'selected' : '' ?>> 該当</option>
            <option value="0" <?php echo ($_SESSION["town_multi_address"])  == 0 ? 'selected' : '' ?>> 該当せず</option>
        </select><br />
        <?php echo $comment_table_fields[11] ?><select name="town_attach_district" id="town_attach_district" size="1">
            <option value="1" <?php echo ($_SESSION["town_attach_district"]) == 1 ? 'selected' : '' ?>> 該当</option>
            <option value="0" <?php echo ($_SESSION["town_attach_district"]) == 0 ? 'selected' : '' ?>> 該当せず</option>
        </select><br />
            <?php echo $comment_table_fields[12] ?><select name="zip_code_multi_town" id="zip_code_multi_town" size="1">
            <option value="1" <?php echo ($_SESSION["zip_code_multi_town"]) == 1 ? 'selected' : '' ?>> 該当</option>
            <option value="0" <?php echo ($_SESSION["zip_code_multi_town"]) == 0 ? 'selected' : '' ?>> 該当せず</option>
        </select><br />
        <?php echo $comment_table_fields[13] ?><select name="update_check" id="update_check" size="1">
            <option value="0" <?php echo ($_SESSION["update_check"] == 0 ) ? 'selected' : '' ?>> 変更なし</option>
            <option value="1" <?php echo ($_SESSION["update_check"] == 1 ) ? 'selected' : '' ?>> 変更あり</option>
            <option value="2" <?php echo ($_SESSION["update_check"] == 2 ) ? 'selected' : '' ?>> 廃止(廃止データのみ使用)</option>
        </select><br />
        <?php echo $comment_table_fields[14] ?>
        <select name="update_reason" id="update_reason" size="1">
            <option value="0" <?php echo ($_SESSION["update_reason"] == 0)  ? 'selected' : '' ?>> 変更なし</option>
            <option value="1" <?php echo ($_SESSION["update_reason"] == 1)  ? 'selected' : '' ?>> 市政・区政・町政・分区・政令指定都市施行</option>
            <option value="2" <?php echo ($_SESSION["update_reason"] == 2)  ? 'selected' : '' ?>> 住居表示の実施</option>
            <option value="3" <?php echo ($_SESSION["update_reason"] == 3)  ? 'selected' : '' ?>> 区画整理</option>
            <option value="4" <?php echo ($_SESSION["update_reason"] == 4)  ? 'selected' : '' ?>> 郵便区調整等</option>
            <option value="5" <?php echo ($_SESSION["update_reason"] == 5)  ? 'selected' : '' ?>> 訂正</option>
            <option value="6" <?php echo ($_SESSION["update_reason"] == 6)  ? 'selected' : '' ?>> 廃止(廃止データのみ使用)</option>
        </select><br />
